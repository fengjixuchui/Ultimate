...
static char * number(char * str, long num, int base, int size, int precision,	int type)
{
	...
	sign = 0;
	if (type&SIGN && num < 0)									//如果是有符号数且获取到的数小于0
	{
		sign='-';												//-符号
		num = -num;												//将负数转换成一般正数
...
int color_printk(unsigned int FRcolor,unsigned int BKcolor,const char * fmt,...)
{
	...
	for(count = 0;count < i || line;count++)					//字符串还未输出完或者当前光标距下一个制表位还需要填充空格
	{
		...
		else if((unsigned char)*(buf + count) == '\b')			//如果确定待显示字符是\b转义字符,那么调整列位置并调用putchar函数打印空格符覆盖之前的字符,如果既不是\n也不是\b,则继续判断其是否为\t转义字符
		{
			...
			{													//列数 = (从左到右横向1440 / 从左到右横向8 - 1) * 从左到右横向8 =>最后一列
			...
			}													//行数 = (从上到下纵向900 / 从上到下纵向16 - 1) *从上到下纵向9 =>最后一行
		...
		{														//8表示一个制表位占用8个显示字符,需要填充的空格符数量 = ((列数 + 8) & ~(8 - 1)) - 列数),比如现在光标在(2, 2),则需要填充的空格符数量 = ((2 + 8) & ~(8 - 1)) - 2) = (8 - 2) = 6
